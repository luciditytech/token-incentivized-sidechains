...
contract Chain is ChainConfig, ReentrancyGuard {
  ...
  StakingBank public stakingBank;
  ...
  ChainConfig(_tokenAddress, _registryAddress, _blocksPerPhase)
  public {
    stakingBank = new StakingBank(_tokenAddress, this);
  }
  ...
  internal
  view
  returns (bool created, bool enabled, uint256 shard, uint256 balance) {
    VerifierRegistry registry = VerifierRegistry(registryAddress);

    ( , , created, enabled, shard) = registry.verifiers(_verifier);

    balance = stakingBank.balances(_verifier);

    return (created, enabled, shard, balance);
  }
